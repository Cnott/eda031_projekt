...
pair<int, vector<string> > InputHandler::parseInput(string input) {
  //Splits input in order to identify commands
  vector<string> parameters = splitBySpace(input);
...
int InputHandler::parseCommand( vector<string> parameters)
  ...
  if (s == "create" && nParam > 2) {
    ...
    } else if ( parameters[1] == "article" &&
                isNumber(parameters[2]) && nParam == 4) {
  ...
  if (s == "get" && nParam == 3 &&
      isNumber(parameters[1])   && isNumber(parameters[2]))
...
/* Is this string strictly a number? */
